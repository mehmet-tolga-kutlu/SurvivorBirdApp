package com.mehmettolgakutlu.survivorbird;
...
public class SurvivorBird extends ApplicationAdapter {
	...
	@Override
	public void create () { // Uygulama başladığında ne olacaksa burada belirtilir.
	...
	@Override
	public void render () { // Oyun/uygulama devam ettiği sürece sürekli çağrılacak olan komut.

		batch.begin();
		batch.draw(background,0,0, Gdx.graphics.getWidth(),Gdx.graphics.getHeight()); // Uzunluk ve yüksekliği bulma.
		...
		if (gameState == 1) {
			...
			if (birdY > 0) {
				...
				gameState = 2; // kuş y ekseni altına düşerse oyunu bitir.
			}


		} else if (gameState == 0) {
			if (Gdx.input.justTouched()) { // Kullanıcı tıkladığında ne olacağını belirtiyoruz.
				gameState = 1;
			}
		} else if (gameState == 2) { // oyunun bitme durumu

			font2.draw(batch,"Game Over! Tap To Play Again!",100,Gdx.graphics.getHeight() / 2); // Game over yazısının konumunu belirlediğimiz yer.

			if (Gdx.input.justTouched()) { // Kullanıcı tıkladığında ne olacağını belirtiyoruz.
				gameState = 1; // oyun bittiğinde kullanıcı tekrar tıklarsa oyun yeniden başlar.
				...
				velocity = 0;
				scoredEnemy = 0;
				score = 0; // Oyun bitince hepsini sıfırlarız.
		...
		font.draw(batch,String.valueOf(score),100,200); // Score nın yazılacagı konum belirtiliyor.
		...
		for (int i = 0; i < numberOfEnemies; i++) {
			...
			if (Intersector.overlaps(birdCircle,enemyCircles[i]) || Intersector.overlaps(birdCircle,enemyCircles2[i]) || Intersector.overlaps(birdCircle,enemyCircles3[i])) { // Çarpışmaları anlamak için kullanılan method.
				gameState = 2; // Oyunun bitme durumu = gamestate == 2
			}
